...
int main(int argc, char *argv[])
{
    ...
    for (int r = 0; r < repeats; r++)
    {
        ...
        HierarchicalNSW<float> *henn = buildHENN(data, size, dim, new L2Space(dim), 4);

        vector<vector<float>> ground_truth = get_ground_truth(queries, num_queries, data, size, dim, k);

        // vector<size_t> ef_values = {10, 20, 40, 80, 160, 320};
        for (int ef = 10; ef <= 300; ef += 20)
        {
            ...
            {
                auto res = henn->searchKnn(queries + j * dim, k);
                while (!res.empty())
                {
    ...
    std::string filename = "../examples/reports/time_recall_henn_lambda=" + std::to_string(lambda) + ".csv";
